...
Partial Class funi
    ...
    Private Sub doci_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        mydata = mychef.top3funload(Session("gKodeDC"), Session("gUserName"))
        ...
        For i = 0 To mydata.Tables(0).Rows.Count - 1
            ...
            If Session("gSound") = "Y" Then
                myhtml.Append("<input onclick='responsiveVoice.speak(" & Chr(34) & "Attention Please. Driver with name : " & mydata.Tables(0).Rows(i).Item("namadriver").ToString.ToLower & ". Expedition : " & mydata.Tables(0).Rows(i).Item("expnamespell").ToString.ToLower & ", Vehicle Number : " & mydata.Tables(0).Rows(i).Item("nokend").ToString.Replace(" ", "") & ", please immediately move your vehicle because the unloading process is complete" & Chr(34) & ");' type='button' value='🔊 Call' style='width:100px' /> &nbsp; ")
                myhtml.Append("<input onclick='responsiveVoice.speak(" & Chr(34) & "Panggilan untuk Pengemudi dengan nama : " & mydata.Tables(0).Rows(i).Item("namadriver").ToString.ToLower & ". Ekspedisi : " & mydata.Tables(0).Rows(i).Item("expnamespell").ToString.ToLower & ", Nomor Kendaraan : " & mydata.Tables(0).Rows(i).Item("nokend").ToString.Replace(" ", "") & ", dipersilahkan segera memindahkan kendaraan Anda karena proses Bongkar Barang sudah selesai" & Chr(34) & "," & Chr(34) & "Indonesian Female" & Chr(34) & ");' type='button' value='🔊 Panggil' style='width:100px' />")
            Else
                myhtml.Append("<input onclick='planQ(" & Chr(34) & "Attention Please. Driver with name : " & mydata.Tables(0).Rows(i).Item("namadriver").ToString.ToLower & ". Expedition : " & mydata.Tables(0).Rows(i).Item("expnamespell").ToString.ToLower & ", Vehicle Number : " & mydata.Tables(0).Rows(i).Item("nokend").ToString.Replace(" ", "") & ", please immediately move your vehicle because the unloading process is complete" & Chr(34) & "," & Chr(34) & Chr(34) & "," & Chr(34) & Session("gKodeDC") & Chr(34) & ");' type='button' value='🔊 Call' style='width:100px' /> &nbsp; ")
                myhtml.Append("<input onclick='planQ(" & Chr(34) & "Panggilan untuk Pengemudi dengan nama : " & mydata.Tables(0).Rows(i).Item("namadriver").ToString.ToLower & ". Ekspedisi : " & mydata.Tables(0).Rows(i).Item("expnamespell").ToString.ToLower & ", Nomor Kendaraan : " & mydata.Tables(0).Rows(i).Item("nokend").ToString.Replace(" ", "") & ", dipersilahkan segera memindahkan kendaraan Anda karena proses Bongkar Barang sudah selesai" & Chr(34) & "," & Chr(34) & "Indonesian Female" & Chr(34) & "," & Chr(34) & Session("gKodeDC") & Chr(34) & ");' type='button' value='🔊 Panggil' style='width:100px' />")
            End If
            myhtml.Append("</td>")
            myhtml.Append("<td Style='text-align: center'>")
            myhtml.Append("<input style='width:200px' type = 'button' id='btnfuni" & i & "' name='btnfuni" & i & "' value='Finish Unloading' runat='server' onclick='JS_click(" & Chr(34) & "FINISHUNLOAD" & Chr(34) & "," & Chr(34) & Session("gKodeDC") & Chr(34) & "," & Chr(34) & mydata.Tables(0).Rows(i).Item("noantrian") & Chr(34) & ")' />")
            myhtml.Append("</td>")
